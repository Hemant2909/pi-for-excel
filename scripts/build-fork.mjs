#!/usr/bin/env node
// Opt-in deployment entry point for Hemant2909's fork.
...
manifest = manifest
  .replace(/<Id>[^<]+<\/Id>/, "<Id>c793f054-66af-43c3-84de-4b775c878269</Id>")
  .replace(/<ProviderName>[^<]+<\/ProviderName>/, "<ProviderName>Hemant2909</ProviderName>")
  .replace(/<DisplayName DefaultValue="[^"]*"\s*\/>/, '<DisplayName DefaultValue="Pi for Excel - Hemant" />')
  .replace(/<SupportUrl DefaultValue="[^"]*"\s*\/>/, '<SupportUrl DefaultValue="https://github.com/Hemant2909/pi-for-excel/issues" />');
